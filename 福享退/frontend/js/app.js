...
setTimeout(() => {
goToPage('payment');
setTimeout(() => renderPaymentDetails(), 100);
}, 500);
} else {
...
let yearOptions = '<option>全部年份</option>';
years.forEach(year => {
yearOptions += '<option>' + year + '年</option>';
});

header.innerHTML = 
'<span class="table-title">缴费明细（共 ' + totalMonths + ' 个月）</span>' +
'<select class="year-select" onchange="filterByYear(this.value)">' +
yearOptions +
'</select>';
...
const personalYearTotal = Math.round(yearTotal * 0.08);

const yearRow = document.createElement('div');
yearRow.className = 'table-row';
yearRow.dataset.year = year;
yearRow.onclick = function() { toggleDetail('detail-' + year); };
yearRow.innerHTML = 
'<div class="row-left">' +
'<div class="row-year">' + year + '年</div>' +
'<div class="row-base">缴费基数：' + avgBase + ' 元</div>' +
'</div>' +
'<div class="row-right">' +
'<div class="row-money">¥ ' + personalYearTotal.toLocaleString() + '</div>' +
...
const avgIndex = (avgBase / 6000).toFixed(2);
...
console.log('✅ 缴费明细渲染完成，共 ' + years.length + ' 年 ' + totalMonths + ' 个月，平均基数：' + Math.round(totalAmount / totalMonths));
}
