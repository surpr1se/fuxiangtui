...
function getPaymentDetailList(userId) { return get('/payment/list', userId ? { userId: userId } : {}).then(function(r) { if (isSuccess(r)) { var d = r.data; r.data = util.normalizePaymentDetails(Array.isArray(d) ? d : ((d && (d.list || d.records)) || [])) } return r }) }
...
function getHistoryList(params) { params = params || {}; var q = { page: params.page || 1, pageSize: params.pageSize || 20 }; if (params.userId) q.userId = params.userId; if (params.openid) q.openid = params.openid; return get('/calculate-result/history', q) }
...
function getPdfList(userId) { return get('/pdf/list', userId ? { userId: userId } : {}) }
